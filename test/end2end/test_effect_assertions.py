...
# below.
# The title shares no word with the utterance below, so speaking the user's
# own slot value cannot satisfy the title assertion. With title="The Matrix"
# against "the matrix" it could, and did: a handler mutated to speak the slot
# instead of the fetched title still passed.
_FAKE_MOVIE = SimpleNamespace(
    title="Synthetic Film Seven",
...
def minicroft():
    ...
    try:
        ...
        # The skill only selects a search result whose title fuzzy-matches the
        # user's own slot value, at a default confidence of 0.8. Against "the
        # matrix" only "The Matrix" itself clears that bar, which is precisely
        # why the old assertion could not tell the fetched title apart from the
        # slot: whenever the handler spoke at all, the two strings were equal.
        # Dropping the bar for this test lets the synthetic payload be selected,
        # so the spoken text can be checked against a title the utterance does
        # not contain. The matcher itself is not under test here.
        loader.instance.settings["match_confidence"] = 0.0
        loader.instance.match_confidence = 0.0
        yield mc
        mc.stop()
    finally:
        movie_patcher.stop()
...
def test_movie_year_speaks_the_fetched_year_and_title(minicroft):
    ...
    assert _FAKE_MOVIE.title.lower() in joined.lower(), (
        f"expected the fetched movie title {_FAKE_MOVIE.title!r} in spoken text, "
        f"got {spoken!r}")
    assert "matrix" not in joined.lower(), (
        f"the handler spoke the utterance's own slot value rather than the "
        f"fetched title, got {spoken!r}")
